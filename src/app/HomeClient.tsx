...
type InitialData = {
  ...
  /** The series `bearbricks` belongs to - what a cold launch lands on. */
  seriesName: string
}
...
export default function HomeClient({ initial }: { initial: InitialData }) {
  return (
    <Suspense fallback={null}>
      <HomePageInner initial={initial} />
    </Suspense>
  )
}

function HomePageInner({ initial }: { initial: InitialData }) {
  const t = useTranslations('home')
  const tc = useTranslations('common')
  const searchParams = useSearchParams()
  const { data: session, status: sessionStatus } = useSession()
  const isAdmin = session?.user?.role === 'ADMIN' || session?.user?.role === 'OWNER'
  const [bearbricks, setBearbricks] = useState<Bearbrick[]>(initial.bearbricks)
  const [allSeries, setAllSeries] = useState<Series[]>(initial.series)
  const [categoryList, setCategoryList] = useState<Category[]>(sortCategoriesOfficial(initial.categories))
  const [selectedSeries, setSelectedSeries] = useState<string>(initial.seriesName)
  ...
  useEffect(() => {
    ...
    // The server already rendered the default series along with the series
    // and category lists, so this only has to decide whether to move off it.
    const isKnown = (name: string) => name === 'all' || initial.series.some((s: Series) => s.name === name)
    if (seriesParam && isKnown(seriesParam)) {
      setSelectedSeries(seriesParam)
    } else {
      const saved = sessionStorage.getItem(SERIES_STORAGE_KEY)
      if (saved && isKnown(saved)) setSelectedSeries(saved)
    }
  ...
  // Cleared the first time it matches, so switching away and back still
  // refetches - it only suppresses the duplicate load of the first paint.
  const serverRendered = useRef<string | null>(initial.seriesName)
  useEffect(() => {
    if (!selectedSeries) return
    sessionStorage.setItem(SERIES_STORAGE_KEY, selectedSeries)
    if (serverRendered.current === selectedSeries) {
      serverRendered.current = null
      return
    }
